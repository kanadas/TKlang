...
transIdent x = case x of
  Ident string -> string
transRelOp :: RelOp -> String
transRelOp x = case x of
  RelOp string -> string
transBasic :: Basic -> String
...
transExpr x = case x of
  ...
  EIdent ident -> do
    ...
    case Map.lookup ident s of
        Just v -> lift $ Right v
        Nothing -> lift $ Left $ "Unknown identifier: " ++ transIdent ident ++ "\n" ++ show x
  ...
  ELambda idents expr -> do
    s <- get
    case idents of
        [ident] -> lift $ Right $ VFun (\v -> evalStateT (transExpr expr) (Map.insert ident v s))
        ident:rest -> lift $ Right $ VFun (\v -> evalStateT (transExpr (ELambda rest expr)) (Map.insert ident v s))
  ...
  ERel expr1 relop expr2 -> do
    ...
    e2 <- transExpr expr2
    case (e1, e2) of
        (VInt i1, VInt i2) -> lift $ Right $ VBool (case transRelOp relop of
            ">" -> i1 > i2
            "<" -> i1 < i2
            "<=" -> i1 <= i2
            ">=" -> i1 >= i2
  ...
  EIf expr1 expr2 expr3 -> do
    e1 <- transExpr expr1
    case e1 of
        VBool i1 -> if i1 then transExpr expr2 else transExpr expr3
